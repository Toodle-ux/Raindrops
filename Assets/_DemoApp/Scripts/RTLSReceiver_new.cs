...
{
    public class RTLSReceiver_new : Singleton<RTLSReceiver_new>
    {
        ...
        private int _currentTrackable = 0;
        ...
        private void ProcessTrackables(List<Trackable> frameTrackables)
        {
            ...
            for (var i = 0; i < frameTrackables.Count; i++)
            {
                ...
                {
                    GameObject trkbl = Instantiate(_trackablePrefabs[_currentTrackable], pos, Quaternion.identity.normalized, gameObject.transform);

                    _currentTrackable = (_currentTrackable + 1) % _trackablePrefabs.Count;
